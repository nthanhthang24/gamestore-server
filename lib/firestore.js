...
const db = {
  ...
  async set(collection, docId, data) {
    ...
    if (transforms.length > 0) {
      ...
      await firestoreRequest('PATCH', `${docPath(collection, docId)}?currentDocument.exists=false`, { fields });
    }
    return docId;
  },
  ...
  async add(collection, data) {
    ...
    const json = await firestoreRequest('POST', url, { fields });
    ...
    if (transforms.length > 0) {
      ...
      await firestoreRequest('POST', commitUrl, {
        writes: [{ transform: { document: json.name, fieldTransforms: transforms } }]
      });
    }
    return { id };
  },
  ...
  async runTransaction(callback) {
    ...
    const tx = {
      ...
      update(ref, data) {
        const fields = toFields(data);
        const transforms = toFieldTransforms(ref.path || '', data);
        if (Object.keys(fields).length > 0) {
          writes.push({
            update: { name: ref._path || ref.path, fields },
            updateMask: { fieldPaths: Object.keys(fields) },
            currentDocument: { exists: true },
          });
        }
        if (transforms.length > 0) {
          writes.push({ transform: { document: ref._path || ref.path, fieldTransforms: transforms } });
        }
      },
    };
    ...
    try {
      ...
      // Rollback — just abandon (Firestore auto-expires)
      throw err;
    }
