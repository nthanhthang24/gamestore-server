...
// Server timestamp marker + atomic increment
const FieldValue = {
...
const db = {
  ...
  // UPDATE document (partial) — supports FieldValue.increment() via field transforms
  async update(collection, docId, data) {
    ...
    for (const [k, v] of Object.entries(data)) {
      if (v && v.__increment !== undefined) {
        transforms.push({
          fieldPath: k,
          increment: toFirestoreValue(v.__increment),
        });
      } else if (v !== undefined) {
        normalFields[k] = toFirestoreValue(v);
      }
    }

    // If there are increments, use the batch commit API (supports field transforms)
    if (transforms.length > 0) {
      ...
      // If also have normal field updates, add an update write
      const writes = [];
      if (Object.keys(normalFields).length > 0) {
        writes.push({
          update: { name: docPath, fields: normalFields },
          updateMask: { fieldPaths: Object.keys(normalFields) },
        });
      }
      writes.push(write);
      await axios.post(
  ...
  // Returns normally on success; throws with response.status=409 if already exists
  async createIfNotExists(collection, docId, data) {
    const body = { fields: toFirestoreFields(data) };
    // currentDocument.exists=false tells Firestore: only write if doc doesn't exist yet
    await axios.patch(
      `${BASE_URL}/${collection}/${docId}?key=${API_KEY}&currentDocument.exists=false`,
      body
    );
  },
